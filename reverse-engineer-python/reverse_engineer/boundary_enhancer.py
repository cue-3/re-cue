    ...
    def __init__(self, verbose: bool = False):
        ...
        self.layer_patterns = {
            'presentation': {
                ...
                'annotation': ['@RestController', '@Controller', '@RequestMapping', '@GetMapping', 
                              '@PostMapping', '@PutMapping', '@DeleteMapping', '@PatchMapping',
                              '@GraphQLController', '@MessageMapping'],
            ...
            'infrastructure': {
                ...
                'annotation': ['@Configuration', '@Bean', '@FeignClient', '@RabbitListener', '@KafkaListener'],
...
class DomainBoundaryDetector:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.domain_patterns = [
            'user', 'account', 'customer', 'order', 'product', 'payment',
            'inventory', 'shipping', 'billing', 'catalog', 'cart', 'checkout',
            'notification', 'authentication', 'authorization', 'profile'
        ]
            ...
            if len(info['components']) >= MIN_DOMAIN_COMPONENTS:  # Only meaningful domains
                boundary = EnhancedBoundary(
                    ...
                    interaction_patterns={
                        target: list(sources) 
                        for target, sources in info['interactions'].items()
                    }
                )
    ...
    def _extract_domain_from_package(self, package: str) -> Optional[str]:
        ...
        for part in parts:
            if part not in ['com', 'org', 'net', 'io', 'app', 'application', 'main', 'java']:
                ...
                if part not in ['controller', 'service', 'repository', 'model', 'dto', 
                               'entity', 'config', 'util', 'helper', 'common']:
                    return part
    ...
    def _detect_ddd_patterns(self, content: str, class_name: str) -> Set[str]:
        ...
        if '@AggregateRoot' in content or (
            '@Entity' in content and 
            any(ann in content for ann in ['@Id', '@GeneratedValue'])
        ):
        ...
        # Domain Service
        if '@DomainService' in content or (
...
class MicroserviceBoundaryDetector:
                ...
                if '<modules>' in content:
                    ...
                    for module_name in module_names:
                        ...
                        if module_path.exists():
                            ...
                            if self.verbose:
                                log_info(f"  Detected microservice from Maven module: {module_name}")
    ...
    def _detect_from_directory_structure(self) -> List[EnhancedBoundary]:
        ...
        for indicator in service_indicators:
            matching_dirs = [
                d for d in self.repo_root.glob(indicator)
...
class BoundaryInteractionAnalyzer:
    ...
    def analyze_interactions(
        self, 
        boundaries: List[EnhancedBoundary],
...
class BoundaryEnhancer:
    ...
    def enhance_boundaries(self) -> Dict[str, Any]:
        ...
        for layer_name, layer in layers.items():
            all_boundaries.append(EnhancedBoundary(
                name=layer.name,
                boundary_type='layer',
        ...
        if self.verbose:
            log_info(f"Enhanced boundary detection complete: {len(all_boundaries)} total boundaries")
