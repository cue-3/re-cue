...
from ...domain import (
    ...
    Actor, SystemBoundary, Relationship,
    CodeLink, TestLink, TraceabilityEntry,
    ImpactedItem, ImpactAnalysis, TraceabilityMatrix,
)
...
class TraceabilityAnalyzer:
    ...
    def analyze(self) -> TraceabilityMatrix:
        ...
        for use_case in self.use_cases:
            ...
            if self.verbose:
                log_info(f"  Linked UC {use_case.id}: {len(entry.code_links)} code, {len(entry.test_links)} tests")
        ...
        if self.verbose:
            log_info(f"Traceability analysis complete: {matrix.total_code_links} code links, {matrix.total_test_links} test links")
    ...
    def analyze_impact(self, changed_file: str) -> ImpactAnalysis:
        ...
        for use_case in self.use_cases:
            ...
            if overlap:
                ...
                analysis.impacted_use_cases.append(ImpactedItem(
                    item_type="use_case",
                    item_id=use_case.id,
    ...
    def _discover_code_components(self):
        ...
        for endpoint in self.endpoints:
            if endpoint.controller:
                self._code_components.append(CodeComponent(
    ...
    def _scan_repository_for_components(self):
        ...
        patterns = {
            'controller': [r'Controller\.', r'controller\.', r'@Controller', r'@RestController'],
            'service': [r'Service\.', r'service\.', r'@Service', r'@Injectable'],
            'repository': [r'Repository\.', r'repository\.', r'@Repository', r'Dao\.'],
            'model': [r'Entity\.', r'entity\.', r'Model\.', r'model\.', r'@Entity'],
            'view': [r'View\.', r'view\.', r'Component\.', r'component\.'],
        }
        ...
        try:
            for file_path in self.repo_root.rglob('*'):
                ...
                if any(part in file_path.parts for part in 
                       ['node_modules', 'vendor', 'target', 'build', '.git', '__pycache__']):
                    continue
    ...
    def _discover_test_files(self):
        ...
        extensions = {'.java', '.py', '.ts', '.js', '.jsx', '.tsx', '.spec.ts', '.spec.js', '.test.ts', '.test.js'}
    ...
    def _create_traceability_entry(self, use_case: UseCase) -> TraceabilityEntry:
        ...
        for component_key, (component, confidence) in matched_components.items():
            if confidence >= MIN_CONFIDENCE_THRESHOLD:
                entry.code_links.append(CodeLink(
                    ...
                    evidence=[f"Keyword match: {', '.join(component.keywords & use_case_keywords)}"]
                ))
        ...
        for test_file in self._test_files:
            ...
            if overlap:
                ...
                entry.test_links.append(TestLink(
    ...
    def _extract_keywords(self, text: str) -> Set[str]:
        ...
        stop_words = {
            'the', 'a', 'an', 'is', 'are', 'to', 'from', 'by', 'for', 'of', 
            'in', 'on', 'at', 'with', 'and', 'or', 'but', 'if', 'then',
            'when', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does',
            'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must',
            'shall', 'can', 'this', 'that', 'these', 'those', 'it', 'its',
            'api', 'http', 'get', 'post', 'put', 'delete', 'patch', 'test',
            'spec', 'java', 'python', 'typescript', 'controller', 'service',
            'repository', 'model', 'view', 'component', 'file', 'class'
        }
    ...
    def _infer_component_type(self, file_path: Path) -> str:
        ...
        if 'controller' in name or '/controllers/' in path_str:
            ...
            return "repository"
        elif 'model' in name or 'entity' in name or '/models/' in path_str or '/entities/' in path_str:
            return "model"
        elif 'view' in name or 'component' in name or '/views/' in path_str or '/components/' in path_str:
            return "view"
        else:
            return "unknown"
    ...
    def _generate_recommendations(self, analysis: ImpactAnalysis) -> List[str]:
        ...
        if analysis.impacted_use_cases:
            direct_count = len([i for i in analysis.impacted_use_cases if i.impact_level == "direct"])
            if direct_count > 0:
