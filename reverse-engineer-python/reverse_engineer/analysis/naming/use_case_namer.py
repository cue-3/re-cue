...
class UseCaseNamer:
    ...
    def _merge_business_terms(self, terms: Dict[str, str]) -> None:
        ...
        for key, value in terms.items():
            ...
            is_verb_key = key_lower in self.verb_mappings or key_lower.endswith(
                ("ate", "ify", "ize", "ish", "ase", "mit", "ove", "ing")
            ) or key_lower in [
                "create", "get", "update", "delete", "find", "search", "list",
                "save", "process", "submit", "send", "receive", "purchase",
                "buy", "sell", "transfer", "assign", "schedule", "cancel"
            ]
    ...
    def _generate_alternatives(
        ...
        for style in other_styles[:self.config.num_alternatives]:
            alt_name = self._generate_styled_name(
                verb, objects, entity, context, style
            )
            alternatives.append(NameSuggestion(
                name=alt_name,
                style=style,
                confidence=0.8,
