...
from ...domain import (
    Endpoint, Actor, SystemBoundary, Relationship
)
...
class RelationshipMapper:
                    ...
                    for actor in self.actors:
                        if self._actor_matches_boundary(actor, source):
                            relationships.append(Relationship(
                                ...
                                identified_from=details[:3] if isinstance(details, list) else [str(details)]
                            ))
    ...
    def map_system_integrations(
        ...
        internal_boundaries = [b for b in self.system_boundaries 
                              if b.type not in ['external_system']]
    ...
    def _determine_access_mechanism(
        self,
        actor: Actor,
        boundary: SystemBoundary
    ) -> str:
    ...
    def _actor_matches_boundary(self, actor: Actor, boundary_name: str) -> bool:
        ...
        return (
            actor_name_lower in boundary_lower or
            boundary_lower in actor_name_lower or
            any(word in boundary_lower for word in actor_name_lower.split())
        )
    
    def _detect_delegation_patterns(
        self,
        user_actors: List[Actor]
        ...
        sorted_actors = sorted(user_actors, 
                              key=lambda a: access_order.get(a.access_level, 3))
        ...
        for i, higher in enumerate(sorted_actors):
            for lower in sorted_actors[i+1:]:
                if higher.access_level != lower.access_level:
                    ...
                    if common_endpoints:
                        communications.append(ActorCommunication(
                            from_actor=higher.name,
                            to_actor=lower.name,
                            communication_type='delegation',
                            mechanism='access_control',
                            endpoints_involved=common_endpoints[:5],
                            identified_from=[f"Access level hierarchy: {higher.access_level} → {lower.access_level}"]
                        ))
    ...
    def _find_common_accessible_endpoints(
        self,
        actor1: Actor,
        actor2: Actor
    ) -> List[str]:
            ...
            if hasattr(domain, 'dependencies'):
                for dep in domain.dependencies:
                    flows.append(DataFlow(
                        ...
                        components=domain.components[:5] if hasattr(domain, 'components') else [],
    ...
    def _communications_to_relationships(
        self,
        communications: List[ActorCommunication]
    ) -> List[Relationship]:
    ...
    def _data_flows_to_relationships(
        self,
            ...
            if len(chain.chain) >= 2:
                ...
                for i in range(len(chain.chain) - 1):
                    relationships.append(Relationship(
                        ...
                        identified_from=[f"Dependency chain from {chain.root} (depth: {chain.depth})"]
                    ))
