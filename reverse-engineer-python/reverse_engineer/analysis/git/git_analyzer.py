...
class GitAnalyzer:
    ...
    def _run_git_command(self, args: List[str], capture_output: bool = True) -> Tuple[str, str, int]:
        """
        ...
        for line in stdout.strip().split('\n'):
            ...
            if len(parts) >= 2:
                ...
                changed_files.append(ChangedFile(
                    ...
                    old_path=old_path if change_type in [FileChangeType.RENAMED, FileChangeType.COPIED] else None
                ))
            ...
            if re.match(r'^[0-9a-f]{40}\s', line):
                ...
                if current_entry.get('commit_sha'):
                    entry = BlameEntry(
                        ...
                        line_end=current_entry.get('line_start', 0) + current_entry.get('line_count', 1) - 1,
    ...
    def get_tags(self) -> List[TagInfo]:
        ...
        """
        stdout, _, code = self._run_git_command(
            ["tag", "-l", "--format=%(refname:short)|%(objectname:short)|%(taggername)|%(taggerdate:unix)|%(contents:subject)"]
        )
        ...
        for line in stdout.strip().split('\n'):
            ...
            if len(parts) >= 2:
                try:
                    timestamp = datetime.fromtimestamp(int(parts[3])) if len(parts) > 3 and parts[3] else None
    ...
    def analyze_changes(
        ...
        """
        log_info(f"Analyzing Git changes from {from_ref or 'working directory'} to {to_ref or 'HEAD'}...", self.verbose)
        ...
        summary = {
            ...
            'files_modified': len([f for f in changed_files if f.change_type == FileChangeType.MODIFIED]),
            'files_deleted': len([f for f in changed_files if f.change_type == FileChangeType.DELETED]),
            'files_renamed': len([f for f in changed_files if f.change_type == FileChangeType.RENAMED]),
    ...
    def filter_files_by_pattern(
        ...
        if include_patterns:
            result = [
                f for f in result
                if any(fnmatch.fnmatch(f.path, p) for p in include_patterns)
            ]
        
        if exclude_patterns:
            result = [
                f for f in result
                if not any(fnmatch.fnmatch(f.path, p) for p in exclude_patterns)
            ]
