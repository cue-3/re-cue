...
class JourneyAnalyzer:
    ...
    def _identify_journeys(self) -> List[UserJourney]:
        ...
        for actor_name in actors_with_use_cases:
            actor_use_cases = [
                uc for uc in self.use_cases 
                if uc.primary_actor.lower() == actor_name.lower()
            ]
    ...
    def _group_use_cases_by_theme(
        self,
        use_cases: List[UseCase]
    ) -> Dict[str, List[UseCase]]:
        ...
        theme_patterns = {
            ...
            'content_management': ['create', 'edit', 'delete', 'update', 'manage', 'content', 'post', 'article'],
    ...
    def _order_use_cases_sequentially(
        self,
        use_cases: List[UseCase]
    ) -> List[UseCase]:
        """
    ...
    def _postconditions_enable_preconditions(
        self,
        uc1: UseCase,
        uc2: UseCase
    ) -> bool:
    ...
    def _generate_journey_description(
        self,
        theme: str,
        use_cases: List[UseCase]
    ) -> str:
    ...
    def _find_endpoints_for_use_case(self, uc: UseCase) -> List[Endpoint]:
        ...
        for endpoint in self.endpoints:
            ...
            if 'create' in uc_name_lower and endpoint.method == 'POST':
                ...
                if any(kw in path_lower for kw in keywords):
                    related.append(endpoint)
            elif ('view' in uc_name_lower or 'get' in uc_name_lower or 'list' in uc_name_lower) and endpoint.method == 'GET':
                if any(kw in path_lower for kw in keywords):
                    related.append(endpoint)
    ...
    def _create_journey_stages(self, journey: UserJourney) -> List[JourneyStage]:
        ...
        for i in range(0, len(use_cases_in_journey), ucs_per_stage):
            ...
            stage_touchpoints = [
                ...
                if tp.use_case_id in stage_uc_ids
            ]
            ...
            stage = JourneyStage(
                ...
                entry_conditions=stage_use_cases[0].preconditions[:2] if stage_use_cases[0].preconditions else [],
                exit_conditions=stage_use_cases[-1].postconditions[:2] if stage_use_cases[-1].postconditions else [],
    ...
    def _identify_cross_boundary_flows(
        self,
