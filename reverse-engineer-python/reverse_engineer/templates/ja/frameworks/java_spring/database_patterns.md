## データベースアクセスパターン - Java Spring

### JPA/Hibernateエンティティマッピング
...
1. **`@Transactional`はサービス層で使用**し、リポジトリやコントローラーでは使用しない
2. **読み取り専用操作には`@Transactional(readOnly = true)`をマーク**する
3. **N+1クエリを回避** - JOIN FETCHまたは@EntityGraphを使用
4. **読み取り専用クエリにはプロジェクションを使用**してメモリ使用量を削減
5. **一括挿入/更新にはバッチ操作**を使用
6. **負荷に応じてコネクションプーリングを適切に設定**
7. **複雑で動的なクエリにはSpecificationsを使用**
8. **デフォルトでLazyロード** - 必要な場合のみEagerフェッチ
9. **頻繁にクエリされる列にインデックスを作成**
10. **hibernate.show_sqlと実行計画でクエリパフォーマンスを監視**
