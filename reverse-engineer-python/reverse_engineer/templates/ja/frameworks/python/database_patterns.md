...
1. **ORMクエリ最適化を使用** - select_related、prefetch_related、only、defer
2. **N+1クエリを防ぐ** - 必要な場合は常に関連オブジェクトを事前読み込み
3. **頻繁にクエリされるフィールドにインデックスを追加** - データベースインデックスを追加
4. **トランザクションを使用** - 関連する操作をトランザクションでラップ
5. **接続プーリング** - 適切なプールサイズを設定
6. **I/Oバウンドアプリには非同期** - 非同期ORM（Tortoise、SQLAlchemy async）を使用
7. **データベース制約** - DBレベルでunique、nullable、defaultを使用
8. **マイグレーション** - マイグレーションツールを使用（Djangoマイグレーション、Alembic）
9. **必要に応じて生のSQL** - ORMが効率的に処理できない複雑なクエリ用
10. **クエリを監視** - 遅いクエリをログに記録して分析
