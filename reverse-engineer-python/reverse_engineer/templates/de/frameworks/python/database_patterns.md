## Datenbankzugriffsmuster - Python
...
#### Benutzerdefinierte Manager und QuerySets
```python
...
### Tortoise ORM (Async für FastAPI)
...
1. **ORM-Abfrageoptimierung verwenden** - select_related, prefetch_related, only, defer
2. **N+1-Abfragen verhindern** - Immer verwandte Objekte vorladen, wenn benötigt
3. **Häufig abgefragte Felder indizieren** - Datenbankindizes hinzufügen
4. **Transaktionen verwenden** - Verwandte Operationen in Transaktionen einschließen
5. **Connection Pooling** - Angemessene Pool-Größen konfigurieren
6. **Async für I/O-gebundene Apps** - Async ORMs verwenden (Tortoise, SQLAlchemy async)
7. **Datenbankbeschränkungen** - unique, nullable, default auf DB-Ebene verwenden
8. **Migrationen** - Migrationswerkzeuge verwenden (Django migrations, Alembic)
9. **Raw SQL bei Bedarf** - Für komplexe Abfragen, die ORM nicht effizient verarbeiten kann
10. **Abfragen überwachen** - Langsame Abfragen protokollieren und analysieren
