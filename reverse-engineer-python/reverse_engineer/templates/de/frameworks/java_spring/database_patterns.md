## Datenbankzugriffsmuster - Java Spring
...
#### Specifications für dynamische Abfragen
```java
...
1. **Verwenden Sie `@Transactional` auf der Service-Ebene**, nicht auf Repository- oder Controller-Ebene
2. **Markieren Sie Nur-Lese-Operationen** mit `@Transactional(readOnly = true)`
3. **Vermeiden Sie N+1-Abfragen** - verwenden Sie JOIN FETCH oder @EntityGraph
4. **Verwenden Sie Projektionen** für Nur-Lese-Abfragen zur Reduzierung der Speichernutzung
5. **Batch-Operationen** für Masseneinfügungen/-aktualisierungen
6. **Konfigurieren Sie Connection Pooling** angemessen für Ihre Last
7. **Verwenden Sie Specifications** für komplexe, dynamische Abfragen
8. **Lazy Loading als Standard** - laden Sie nur bei Bedarf eager
9. **Indizieren Sie häufig abgefragte Spalten** in der Datenbank
10. **Überwachen Sie die Abfrageleistung** mit hibernate.show_sql und Explain-Plänen

### Gängige Muster

#### Repository-Pattern-Erweiterung
```java
@Repository
