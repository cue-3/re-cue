## Modèles d'Accès aux Bases de Données - Python
...
#### Gestionnaires et QuerySets Personnalisés
```python
...
### Tortoise ORM (Asynchrone pour FastAPI)
...
1. **Utiliser l'optimisation des requêtes ORM** - select_related, prefetch_related, only, defer
2. **Prévenir les requêtes N+1** - Toujours charger les objets liés en avance quand nécessaire
3. **Indexer les champs fréquemment interrogés** - Ajouter des index de base de données
4. **Utiliser les transactions** - Encapsuler les opérations liées dans des transactions
5. **Pool de connexions** - Configurer des tailles de pool appropriées
6. **Asynchrone pour les applications liées aux E/S** - Utiliser des ORM asynchrones (Tortoise, SQLAlchemy async)
7. **Contraintes de base de données** - Utiliser unique, nullable, default au niveau BD
8. **Migrations** - Utiliser des outils de migration (migrations Django, Alembic)
9. **SQL brut quand nécessaire** - Pour les requêtes complexes que l'ORM ne peut pas gérer efficacement
10. **Surveiller les requêtes** - Journaliser et analyser les requêtes lentes
