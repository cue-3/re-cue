## Patrones de Acceso a Base de Datos - Python
...
### Tortoise ORM (Asíncrono para FastAPI)
...
1. **Usar optimización de consultas ORM** - select_related, prefetch_related, only, defer
2. **Prevenir consultas N+1** - Siempre cargar objetos relacionados cuando sea necesario
3. **Indexar campos consultados frecuentemente** - Agregar índices de base de datos
4. **Usar transacciones** - Envolver operaciones relacionadas en transacciones
5. **Pool de conexiones** - Configurar tamaños de pool apropiados
6. **Asíncrono para aplicaciones limitadas por I/O** - Usar ORMs asíncronos (Tortoise, SQLAlchemy async)
7. **Restricciones de base de datos** - Usar unique, nullable, default a nivel de BD
8. **Migraciones** - Usar herramientas de migración (Django migrations, Alembic)
9. **SQL crudo cuando sea necesario** - Para consultas complejas que el ORM no puede manejar eficientemente
10. **Monitorear consultas** - Registrar y analizar consultas lentas
