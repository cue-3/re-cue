## Patrones de Acceso a Base de Datos - Java Spring
...
#### Especificaciones para Consultas Dinámicas
```java
...
1. **Usa `@Transactional` en la capa de servicio**, no en el repositorio o controlador
2. **Marca las operaciones de solo lectura** con `@Transactional(readOnly = true)`
3. **Evita consultas N+1** - usa JOIN FETCH o @EntityGraph
4. **Usa proyecciones** para consultas de solo lectura para reducir el uso de memoria
5. **Operaciones por lotes** para inserciones/actualizaciones masivas
6. **Configura el pool de conexiones** apropiadamente para tu carga
7. **Usa Specifications** para consultas complejas y dinámicas
8. **Carga lazy por defecto** - carga eager solo cuando sea necesario
9. **Indexa las columnas consultadas frecuentemente** en la base de datos
10. **Monitorea el rendimiento de las consultas** con hibernate.show_sql y planes de ejecución

### Patrones Comunes

#### Extensión del Patrón Repository
```java
@Repository
