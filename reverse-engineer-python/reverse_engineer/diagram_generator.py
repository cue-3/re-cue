...
class SequenceDiagramGenerator(BaseDiagramGenerator):
    ...
    def generate(self) -> str:
        ...
        for actor_name, actor_eps in list(actor_endpoints.items())[:self.config.max_actors_per_diagram]:
    ...
    def _generate_actor_sequence_diagram(self, actor_name: str, endpoints: List) -> str:
        ...
        for ep in endpoints[:5]:  # Limit to 5 interactions
            ...
            controller = getattr(ep, 'controller', 'Controller')
            ...
            if method in ['POST', 'PUT', 'PATCH', 'DELETE']:
                lines.append(f"    Service->>+DB: Update Data")
                lines.append(f"    DB-->>-Service: Confirm")
            else:
                lines.append(f"    Service->>+DB: Query Data")
                lines.append(f"    DB-->>-Service: Return Data")
...
class ComponentDiagramGenerator(BaseDiagramGenerator):
    ...
    def _generate_default_component_diagram(self) -> str:
        ...
        for i, ctrl in enumerate(controllers[:5]):
            ctrl_name = ctrl if isinstance(ctrl, str) else getattr(ctrl, 'name', f'Controller{i+1}')
...
class DiagramGenerator:
    ...
    def generate_all_diagrams(self) -> str:
        ...
        sections.append("")
        sections.append("This document contains visual diagrams generated from the analyzed codebase.")
        sections.append("")
        sections.append("---")
        sections.append("")
