    ...
    def _convert_headers(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if match:
                ...
                result.append(f'<h{level} id="{heading_id}">{content}</h{level}>')
            else:
                result.append(line)
...
<body>
    <div class="container">
        ...
        <main class="content" id="content">
            ...
            <footer class="page-footer">
                <p>Generated by <a href="https://github.com/cue-3/re-cue" target="_blank">RE-cue</a> on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
            </footer>
        </main>
    </div>
    ...
    def _generate_toc_html(self, toc_items: List[Dict[str, Any]]) -> str:
        ...
        for item in toc_items:
            ...
            html_parts.append(
                f"<li class='{level_class}'><a href='#{item['id']}'>{item['text']}</a></li>"
            )
    ...
    def _create_index_page(self, results: List[HTMLExportResult]):
        ...
        for category, docs in categories.items():
            if docs:
                ...
                for doc in docs:
                    rel_path = doc.file_path.name if doc.file_path else ""
                    sections_html.append(
                        f'<li><a href="{rel_path}" class="document-link">{doc.title}</a></li>'
                    )
