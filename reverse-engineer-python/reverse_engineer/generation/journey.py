...
- **User Story Mapping**: Detailed stories linked to journey stages"""
    ...
    def _generate_journey_details(self) -> str:
        ...
        for journey in self.journey_map.journeys:
            ...
            if journey.boundaries_crossed:
                lines.append("")
                lines.append(f"**System Boundaries Involved**: {', '.join(journey.boundaries_crossed)}")
            ...
            if journey.touchpoints:
                ...
                if len(journey.touchpoints) > 15:
                    lines.append(f"| ... | *{len(journey.touchpoints) - 15} more touchpoints* | ... | ... |")
    ...
    def _generate_epics_section(self) -> str:
        ...
        lines.append("")
        lines.append("Epics are generated from user journeys, representing major features that can be broken down into user stories.")
            ...
            for flow in self.journey_map.cross_boundary_flows[:10]:
                if " → " in flow:
                    ...
                    if len(parts) >= 2:
                        ...
                        source = parts[0].split("flow: ")[-1].strip() if "flow: " in parts[0] else parts[0].strip()
    ...
    def _generate_recommendations(self) -> str:
        ...
        if complex_journeys:
            lines.append(f"{rec_num}. 🔴 **Simplify Complex Journeys**: {len(complex_journeys)} journey(s) identified as complex")
        ...
        if high_touchpoint_journeys:
            lines.append(f"{rec_num}. 🟡 **Review High-Interaction Journeys**: {len(high_touchpoint_journeys)} journey(s) have more than 10 touchpoints")
            rec_num += 1
        
        # Check for cross-boundary flows
        if len(self.journey_map.cross_boundary_flows) > 5:
            lines.append(f"{rec_num}. 🟠 **Analyze Boundary Interactions**: {len(self.journey_map.cross_boundary_flows)} cross-boundary flows detected - consider API gateway optimization")
            rec_num += 1
