...
class IntegrationTestGenerator(BaseGenerator):
    ...
    def _create_error_scenarios(self, use_case: UseCase) -> List[TestScenario]:
        ...
        for i, extension in enumerate(use_case.extensions, 1):
            ...
            scenarios.append(TestScenario(
                ...
                postconditions=["System remains in valid state", "Error is logged appropriately"],
    ...
    def _create_boundary_scenarios(self, use_case: UseCase) -> List[TestScenario]:
        ...
        has_data_constraints = any(
            any(keyword in cond.lower() for keyword in ['valid', 'minimum', 'maximum', 'required', 'format'])
        ...
        if has_data_constraints:
            ...
            scenarios.append(TestScenario(
                ...
                preconditions=use_case.preconditions.copy(),
                test_steps=[
                    TestStep(step_number=1, action="Test with minimum valid values", expected_result="System accepts input"),
                    TestStep(step_number=2, action="Test with maximum valid values", expected_result="System accepts input"),
                    TestStep(step_number=3, action="Test with values just below minimum", expected_result="System rejects with validation error"),
                    TestStep(step_number=4, action="Test with values just above maximum", expected_result="System rejects with validation error"),
                ],
                expected_outcome="System correctly validates input boundaries",
                postconditions=["Valid data is accepted", "Invalid data is rejected with clear error messages"],
    ...
    def _create_security_scenario(self, use_case: UseCase) -> TestScenario:
        ...
        return TestScenario(
            ...
            test_steps=[
                TestStep(step_number=1, action="Attempt access without authentication", expected_result="Access denied with 401 status"),
                TestStep(step_number=2, action="Attempt access with invalid token", expected_result="Access denied with 401 status"),
                TestStep(step_number=3, action="Attempt access with expired token", expected_result="Access denied with 401 status"),
                TestStep(step_number=4, action="Attempt access with insufficient permissions", expected_result="Access denied with 403 status"),
            ],
            ...
            test_data=[],
            tags=["security", "authentication", "authorization", self._extract_domain_tag(use_case)]
        )
    ...
    def _build_coverage_mappings(self, suite: IntegrationTestSuite) -> List[CoverageMapping]:
        ...
        for use_case in self.analyzer.use_cases:
            ...
            related_scenarios = [
                ts.id for ts in suite.test_scenarios 
                if ts.use_case_id == use_case.id
            ]
            ...
            mappings.append(CoverageMapping(
                use_case_id=use_case.id,
                use_case_name=use_case.name,
    ...
    def _generate_api_tests_section(self) -> str:
        ...
        for endpoint, tests in by_endpoint.items():
            ...
            for test in tests:
                ...
                lines.append("\n**Sample Request**:")
                lines.append("```bash")
                auth_header = ' -H "Authorization: Bearer ${TOKEN}"' if test.authentication_required else ''
...
```""")
    ...
    def _generate_coverage_section(self) -> str:
        ...
        for mapping in self.test_suite.coverage_mappings:
            ...
            coverage_bar = "█" * int(mapping.coverage_percentage / 10) + "░" * (10 - int(mapping.coverage_percentage / 10))
...
```"""
    ...
    def _generate_recommendations(self) -> str:
        ...
        security_tests = len([s for s in self.test_suite.test_scenarios if s.test_type == 'security'])
        
        recommendations.append("\n### Priority Actions\n")
        
        if total_tests < len(self.analyzer.use_cases):
            recommendations.append("1. ⚠️ **Increase Test Coverage**: Some use cases have limited test scenarios")
        
        if security_tests == 0 and any(ep.authenticated for ep in self.analyzer.endpoints):
            recommendations.append("1. 🔒 **Add Security Tests**: Authenticated endpoints detected but no security tests generated")
        
        if len(self.test_suite.e2e_flows) < 2:
            recommendations.append("1. 🔄 **Create E2E Flows**: Add more comprehensive end-to-end test flows")
...
- Keep test documentation in sync with implementation""")
    ...
    def _extract_domain_tag(self, use_case: UseCase) -> str:
        ...
        # Skip common verbs
        skip_words = {'create', 'update', 'delete', 'view', 'list', 'get', 'add', 'remove', 'the', 'a', 'an'}
    ...
    def _extract_keywords(self, text: str) -> set:
        ...
        stop_words = {'the', 'a', 'an', 'is', 'are', 'to', 'from', 'by', 'for', 'of', 'in', 'on', 'at'}
    ...
    def _find_uncovered_aspects(self, use_case: UseCase, scenarios: List[TestScenario]) -> List[str]:
        ...
        if len(use_case.extensions) > len([s for s in use_case_scenarios if s.test_type == 'error_case']):
