...
class FourPlusOneDocGenerator(BaseGenerator):
    ...
    def generate(self) -> str:
        ...
        variables = {
            ...
            'KEY_COMPONENTS': ', '.join([s.name for s in self.analyzer.services[:3]]) if self.analyzer.services else 'N/A',
            ...
            # Concurrency
            'TRANSACTION_COUNT': str(len([e for e in self.analyzer.endpoints if e.method in ['POST', 'PUT', 'DELETE']])),
            'WRITE_OPERATION_COUNT': str(len([e for e in self.analyzer.endpoints if e.method in ['POST', 'PUT', 'DELETE']])),
            'READ_OPERATION_COUNT': str(len([e for e in self.analyzer.endpoints if e.method == 'GET'])),
            ...
            'LANGUAGE': 'java' if any('java' in str(f) for f in self.analyzer.repo_root.rglob('*.java')) else 'python',
            ...
            'ITEM_1': 'Connection pool: 20',
            'ITEM_2': 'Timeout: 30s',
            'ITEM_3': 'Max connections: 100',
            ...
            'ACTOR_2': self.analyzer.actors[1].name if len(self.analyzer.actors) > 1 else 'Administrator',
