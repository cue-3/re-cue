    ...
    def _generate_json(self) -> str:
        ...
        data = {
            ...
            "summary": {
                ...
                "average_implementation_coverage": round(self.matrix.average_implementation_coverage, 1),
...
| ❌ Unverified | {len([e for e in self.matrix.entries if e.verification_status == 'unverified'])} |"""
    ...
    def _generate_matrix_table(self) -> str:
        ...
        lines.append("")
        lines.append("| Use Case ID | Name | Actor | Code Links | Test Links | Impl % | Test % | Status |")
        lines.append("|-------------|------|-------|------------|------------|--------|--------|--------|")
    ...
    def _generate_detailed_entries(self) -> str:
        ...
        for entry in self.matrix.entries:
            ...
            lines.append(f"**Implementation Status**: {entry.implementation_status.replace('_', ' ').title()}  ")
    ...
    def _generate_coverage_analysis(self) -> str:
        ...
        lines.append("|----------------|-------|-----------|")
        lines.append(f"| High (≥80%) | {len(high_impl)} | {', '.join(e.use_case_id for e in high_impl[:5])}{'...' if len(high_impl) > 5 else ''} |")
        lines.append(f"| Medium (40-79%) | {len(medium_impl)} | {', '.join(e.use_case_id for e in medium_impl[:5])}{'...' if len(medium_impl) > 5 else ''} |")
        lines.append(f"| Low (<40%) | {len(low_impl)} | {', '.join(e.use_case_id for e in low_impl[:5])}{'...' if len(low_impl) > 5 else ''} |")
        lines.append(f"| None (0%) | {len(no_impl)} | {', '.join(e.use_case_id for e in no_impl[:5])}{'...' if len(no_impl) > 5 else ''} |")
        ...
        lines.append("| Coverage Level | Count | Use Cases |")
        lines.append("|----------------|-------|-----------|")
        lines.append(f"| High (≥80%) | {len(high_test)} | {', '.join(e.use_case_id for e in high_test[:5])}{'...' if len(high_test) > 5 else ''} |")
        lines.append(f"| Medium (40-79%) | {len(medium_test)} | {', '.join(e.use_case_id for e in medium_test[:5])}{'...' if len(medium_test) > 5 else ''} |")
        lines.append(f"| Low (<40%) | {len(low_test)} | {', '.join(e.use_case_id for e in low_test[:5])}{'...' if len(low_test) > 5 else ''} |")
        lines.append(f"| None (0%) | {len(no_test)} | {', '.join(e.use_case_id for e in no_test[:5])}{'...' if len(no_test) > 5 else ''} |")
    ...
    def _generate_gap_analysis(self) -> str:
        ...
        if low_coverage_with_links:
            lines.append("The following use cases have implementation or test coverage below 50%:\n")
    ...
    def _generate_recommendations(self) -> str:
        ...
        unimplemented = self.matrix.get_unimplemented_use_cases()
        if unimplemented:
            lines.append(f"{priority_num}. 🔴 **Implement Missing Use Cases**: {len(unimplemented)} use case(s) have no linked code")
            priority_num += 1
        
        # Check for untested use cases
        untested = self.matrix.get_untested_use_cases()
        if untested:
            lines.append(f"{priority_num}. 🟡 **Add Test Coverage**: {len(untested)} use case(s) have no linked tests")
            priority_num += 1
        
        # Check for low coverage
        low_coverage = self.matrix.get_low_coverage_use_cases(50.0)
        if low_coverage:
            lines.append(f"{priority_num}. 🟠 **Improve Coverage**: {len(low_coverage)} use case(s) have coverage below 50%")
            priority_num += 1
        
        # Check for partial verification
        partial = [e for e in self.matrix.entries if e.verification_status == 'partial']
        if partial:
            lines.append(f"{priority_num}. 🔵 **Complete Verification**: {len(partial)} use case(s) are partially verified")
            priority_num += 1
        
        if priority_num == 1:
            lines.append("✅ No critical issues found. All use cases are well-implemented and tested.")
    ...
    def generate_impact_section(self, analysis: ImpactAnalysis) -> str:
        ...
        if analysis.impacted_use_cases:
            ...
            for item in analysis.impacted_use_cases:
                ...
                lines.append(f"| {impact_icon} {item.item_id}: {item.item_name} | {item.impact_level} | {item.reason} |")
        else:
