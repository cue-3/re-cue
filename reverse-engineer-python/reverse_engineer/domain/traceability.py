...
class ImpactAnalysis:
    ...
    def total_impacts(self) -> int:
        ...
        return (len(self.impacted_use_cases) + 
    ...
    def assess_risk(self) -> str:
        ...
        direct_use_cases = len([i for i in self.impacted_use_cases 
                                if i.impact_level == "direct"])
...
class TraceabilityMatrix:
    ...
    def compute_metrics(self):
        ...
        if self.entries:
            self.average_implementation_coverage = (
                sum(e.implementation_coverage for e in self.entries) / len(self.entries)
            )
            self.average_test_coverage = (
                sum(e.test_coverage for e in self.entries) / len(self.entries)
            )
