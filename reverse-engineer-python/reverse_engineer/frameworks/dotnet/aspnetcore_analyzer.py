...
class DotNetAspNetCoreAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, repo_root: Path, verbose: bool = False):
        ...
        self.endpoint_attributes = [
            ...
            "[HttpDelete]", "[HttpPatch]", "[Route]"
        ]
        
        self.security_attributes = [
            "[Authorize]", "[AllowAnonymous]",
            "[RequireHttps]", "[ValidateAntiForgeryToken]"
        ]
        ...
        for di_file in di_files:
            ...
            try:
                ...
                for match in re.finditer(scoped_single_pattern, content):
                    ...
                    if not any(s['implementation'] == impl and s['lifetime'] == 'Scoped' for s in self.di_services):
                        self.di_services.append({
                            'interface': impl,
                            'implementation': impl,
                            'lifetime': 'Scoped'
                ...
                for match in re.finditer(singleton_single_pattern, content):
                    impl = match.group(1).strip()
                    if not any(s['implementation'] == impl and s['lifetime'] == 'Singleton' for s in self.di_services):
                        self.di_services.append({
                            'interface': impl,
                            'implementation': impl,
                            'lifetime': 'Singleton'
                ...
                for match in re.finditer(transient_single_pattern, content):
                    impl = match.group(1).strip()
                    if not any(s['implementation'] == impl and s['lifetime'] == 'Transient' for s in self.di_services):
                        self.di_services.append({
                            'interface': impl,
                            'implementation': impl,
                            'lifetime': 'Transient'
    ...
    def _analyze_controller_file(self, file_path: Path):
        ...
        if base_route_match:
            base_path = "/" + base_route_match.group(1).replace("[controller]", controller_name.lower())
        ...
        http_patterns = [
            (r'\[HttpGet(?:\(["\']([^"\']*)["\'](?:,\s*Name\s*=\s*["\'][^"\']*["\'])?\))?\]', 'GET'),
            (r'\[HttpPost(?:\(["\']([^"\']*)["\'](?:,\s*Name\s*=\s*["\'][^"\']*["\'])?\))?\]', 'POST'),
            (r'\[HttpPut(?:\(["\']([^"\']*)["\'](?:,\s*Name\s*=\s*["\'][^"\']*["\'])?\))?\]', 'PUT'),
            (r'\[HttpDelete(?:\(["\']([^"\']*)["\'](?:,\s*Name\s*=\s*["\'][^"\']*["\'])?\))?\]', 'DELETE'),
            (r'\[HttpPatch(?:\(["\']([^"\']*)["\'](?:,\s*Name\s*=\s*["\'][^"\']*["\'])?\))?\]', 'PATCH'),
        ]
    ...
    def _discover_razor_page_endpoints(self):
        ...
        for razor_model in razor_models:
            ...
            try:
                ...
                page_path = "/" + str(rel_path.parent.relative_to("Pages") if "Pages" in rel_path.parts else rel_path.parent).replace("\\", "/")
    ...
    def _analyze_model_file(self, file_path: Path):
        ...
        for match in re.finditer(class_pattern, content):
            ...
            if any(suffix in class_name for suffix in ['Context', 'Service', 'Controller', 'Repository']):
                continue
            
            # Count properties
            property_pattern = r'public\s+(?:virtual\s+)?(?:required\s+)?[\w<>\[\]?,\s]+\s+(\w+)\s*{\s*get'
            properties = re.findall(property_pattern, content)
            
            # Count Entity Framework attributes
            ef_attributes = len(re.findall(r'\[(Key|Column|ForeignKey|Required|MaxLength|Table)\]', content))
            ...
            if properties or ef_attributes > 0:
                ...
                if not any(m.name == class_name for m in self.models):
                    ...
                    log_info(f"  Found model: {class_name} ({len(properties)} properties)", self.verbose)
    ...
    def discover_actors(self) -> List[Actor]:
        ...
        for cs_file in cs_files:
            ...
            try:
                ...
                role_matches = re.findall(r'\[Authorize\([^)]*Roles\s*=\s*["\']([^"\']+)["\'][^)]*\)\]', content)
                for roles_str in role_matches:
                    for role in roles_str.split(','):
                        roles_found.add(role.strip())
                
                # Extract policies from [Authorize(Policy = "...")]
                policy_matches = re.findall(r'\[Authorize\([^)]*Policy\s*=\s*["\']([^"\']+)["\'][^)]*\)\]', content)
                policies_found.update(policy_matches)
                
                # Find role definitions in IdentityRole
                identity_roles = re.findall(r'new\s+IdentityRole\s*\(\s*["\']([^"\']+)["\']', content)
                roles_found.update(identity_roles)
                
                # Find role constants
                role_constants = re.findall(r'const\s+string\s+\w*[Rr]ole\w*\s*=\s*["\']([^"\']+)["\']', content)
    ...
    def discover_system_boundaries(self) -> List[SystemBoundary]:
        ...
        if controller_files:
            self.boundaries.append(SystemBoundary(
                name="API Layer",
                type="presentation",
    ...
    def _generate_use_case_name(self, endpoint: Endpoint) -> str:
        ...
        parts = [p for p in endpoint.path.split('/') if p and not p.startswith('{') and not p.startswith(':')]
    ...
    def get_model_patterns(self) -> dict:
        ...
        return {
            ...
            'property_patterns': [
                r'public\s+(?:virtual\s+)?[\w<>\[\]?,\s]+\s+(\w+)\s*{\s*get'
            ]
        }
