...
class RubyRailsAnalyzer(BaseAnalyzer):
    ...
    def _analyze_model_file(self, file_path: Path):
        ...
        try:
            ...
            associations = len(re.findall(r'(has_many|has_one|belongs_to|has_and_belongs_to_many)', content))
    ...
    def discover_actors(self) -> List[Actor]:
        ...
        if self.controllers_path.exists():
            has_admin = any(
                ...
                for f in self.controllers_path.rglob("*_controller.rb")
            )
    ...
    def _extract_controller_use_cases(self, controller_file: Path):
        ...
        try:
            ...
            for action in actions:
                ...
                use_case_name = crud_mappings.get(action, f'{action.replace("_", " ").title()} {resource_name}')
