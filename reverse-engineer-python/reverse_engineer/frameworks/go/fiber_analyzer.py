"""
Fiber Framework Analyzer for Go.

Analyzes Fiber (github.com/gofiber/fiber) applications to extract:
...
class FiberAnalyzer(BaseAnalyzer):
    """Analyzer for Fiber framework applications."""

    framework_id = "go_fiber"
    ...
    def discover_endpoints(self) -> list[Endpoint]:
        """Discover Fiber route endpoints."""
        log_info("Discovering Fiber routes...", self.verbose)
    ...
    def _analyze_route_file(self, file_path: Path):
        """Analyze a single Go file for Fiber endpoints."""
        try:
        ...
        # Skip if doesn't use Fiber
        if "fiber" not in content.lower():
            return
        ...
        # Find Fiber route definitions
        # Patterns: app.Get("/path", handler), app.Post("/path", middleware, handler)
        patterns = [
            r'(?:app|fiber)\.(?:Get|Post|Put|Delete|Patch)\s*\(\s*"([^"]+)"',
            r'(?:app|fiber)\.Add\s*\(\s*"(GET|POST|PUT|DELETE|PATCH)"\s*,\s*"([^"]+)"',
        ]
        ...
        for i, line in enumerate(lines):
            ...
            if method_match:
                ...
                # Try pattern 1: app.Get("/path", ...)
                match = re.search(patterns[0], line)
                if match:
                    path = match.group(1)
                    method_func = re.search(r"\.(\w+)\s*\(", line)
                    if method_func:
                        # Fiber uses capitalized method names (Get, Post, etc.)
                        method = method_func.group(1).upper()

                # Try pattern 2: app.Add("GET", "/path", ...)
                if not match:
                    match = re.search(
    ...
    def _check_authentication(self, lines: list[str], current_line: int) -> bool:
        ...
        # Pattern: app.Post("/path", authRequired, handler)
