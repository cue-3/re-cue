"""
Gin Framework Analyzer for Go.

Analyzes Gin (github.com/gin-gonic/gin) applications to extract:
...
class GinAnalyzer(BaseAnalyzer):
    """Analyzer for Gin framework applications."""

    framework_id = "go_gin"

    def __init__(self, repo_root: Path, verbose: bool = False):
        """Initialize the Gin analyzer."""
    ...
    def discover_endpoints(self) -> list[Endpoint]:
        """Discover Gin route endpoints."""
        log_info("Discovering Gin routes...", self.verbose)
    ...
    def _analyze_route_file(self, file_path: Path):
        """Analyze a single Go file for Gin endpoints."""
        try:
        ...
        # Skip if doesn't use Gin
        if "gin" not in content.lower():
            return
        ...
        # Find Gin route definitions
        # Patterns: r.GET("/path", handler), router.POST("/path", middleware, handler)
        patterns = [
            r'(?:r|router|engine|app)\.(?:GET|POST|PUT|DELETE|PATCH)\s*\(\s*"([^"]+)"',
            r'(?:r|router|engine|app)\.Handle\s*\(\s*"(GET|POST|PUT|DELETE|PATCH)"\s*,\s*"([^"]+)"',
        ]

        lines = content.split("\n")
        for i, line in enumerate(lines):
            # Check for HTTP method patterns
            method_match = re.search(r"\.(?:GET|POST|PUT|DELETE|PATCH|Handle)\s*\(", line)

            if method_match:
                method = None
                path = None

                # Try pattern 1: r.GET("/path", ...)
                ...
                # Try pattern 2: r.Handle("GET", "/path", ...)
                if not match:
                    match = re.search(
                        r'\.Handle\s*\(\s*"(GET|POST|PUT|DELETE|PATCH)"\s*,\s*"([^"]+)"', line
                    )
                    if match:
    ...
    def _check_authentication(self, lines: list[str], current_line: int) -> bool:
        ...
        # Pattern: r.POST("/path", authMiddleware, handler)
